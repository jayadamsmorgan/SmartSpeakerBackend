...
struct AuthService {
    ...
    func register(req: Request, authDTO: AuthDTO) async throws -> ClientTokenReponse {
    ...
    func authenticate(req: Request, authDTO: AuthDTO) async throws -> ClientTokenReponse {
        let logger = req.logger
        var user: User?
